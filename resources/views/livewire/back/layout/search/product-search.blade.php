<div class="mb-0 input-group">
    <input type="search" wire:model.debounce.300ms="search" class="form-control @error('publisher_id') is-invalid @enderror" id="product-input" placeholder="{{ __('back/review.odaberi_proizvod') }}" autocomplete="off">
            ...
            <div id="myInputautocomplete-list" class="autocomplete-items">
                @foreach($search_results as $product)
                    <div style="cursor: pointer;border-bottom: 1px solid #d7d7d7;padding-bottom: 10px;padding-left: 10px;font-size: 16px" wire:click="addProduct('{{ $product->id }}')">
                        <small class="font-weight-lighter">{{ __('back/review.naziv') }}: <strong>{{ $product->name }}</small>
                    </div>
                @endforeach
            </div>
        </div>
