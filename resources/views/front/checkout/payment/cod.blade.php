<form name="pay" class="needs-validation w-100" action="{{ route('checkout') }}" novalidate method="GET">
    ...
    <div class="w-50 ps-2">
        <button class="btn btn-primary d-block w-100" type="submit"><span class="d-none d-sm-inline">{{ __('front/cart.dovrsi_kupnju') }}</span><span class="d-inline d-sm-none">{{ __('front/cart.dovrsi_kupnju') }}</span><i class="ci-arrow-right  ms-1"></i></button>
    </div>
    </div>
</form>
